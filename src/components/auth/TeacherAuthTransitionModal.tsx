...
interface TeacherAuthTransitionModalProps {
  isOpen: boolean;
  teacherIdentifier?: string; // 'tekchas', 'choeurn-tekchas', 'nun.langdy', 'ten.chandara', etc.
  customTeacherData?: Partial<TeacherProfile>;
  onComplete: () => void;
}

export const TeacherAuthTransitionModal: React.FC<TeacherAuthTransitionModalProps> = ({
  isOpen,
  teacherIdentifier = 'tekchas',
  customTeacherData,
  onComplete,
}) => {
  ...
  // Match active teacher from TEACHERS_DATA
  const teacher: TeacherProfile = useMemo(() => {
    const idLower = (teacherIdentifier || '').toLowerCase();
    
    if (
      idLower.includes('tekchas') ||
      idLower.includes('choeurn') ||
      idLower.includes('asst') ||
      idLower === 'teacher-choeurn-tekchas'
    ) {
      return TEACHERS_DATA.find(t => t.id === 'teacher-choeurn-tekchas') || TEACHERS_DATA[2];
    }
    
    if (
      idLower.includes('chandara') ||
      idLower.includes('ten') ||
      idLower === 'teacher-ten-chandara'
    ) {
      return TEACHERS_DATA.find(t => t.id === 'teacher-ten-chandara') || TEACHERS_DATA[1];
    }

    const found = TEACHERS_DATA.find(t => t.id === 'teacher-nun-langdy');
    return found || TEACHERS_DATA[0];
  }, [teacherIdentifier]);
  ...
  return createPortal(
    ...
    >
      {/* ========================================================================= */}
      {/* 1. LAYERED ORGANIC BLUR SHAPES & LUMINOUS AMBIENT GLOWS                   */}
      {/* ========================================================================= */}
      <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
        {/* Soft Central Pink Glow Orb */}
        ...
        {/* Secondary Soft Magenta Atmospheric Pool */}
      ...
      {/* ========================================================================= */}
      {/* 2. BORDERLESS VERTICAL-SPLIT LOADING COMPOSITION                          */}
      {/* ========================================================================= */}
      <div className="relative z-10 flex items-center justify-center max-w-2xl px-6 w-full">
        ...
        <div className="relative w-16 sm:w-24 h-40 sm:h-52 overflow-hidden flex items-center justify-end shrink-0">
          {/* Full gear rotating behind vertical beam seam */}
          <div
          ...
          >
            ...
            >
              {/* Industrial Gear with 8 Smooth Cogs */}
              <path
        ...
        {/* CENTER VERTICAL SEAM DIVIDER (Pure gradient beam with soft luminous bloom, no hard borders) */}
